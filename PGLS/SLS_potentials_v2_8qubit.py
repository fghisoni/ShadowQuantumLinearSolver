...
fname = f'{16}x{16}sites_potential.txt'
...
b = np.load(f'{16}x{16}_bnorm.npy')
...
while condition==False:
    ...
    if int(len(cost_function_values))%10==0:
        fname = f'8qubit/Potential_{n}qubits_{shadow_error}serr_costs_{seed}seed_Adam'
        with open(fname+'.txt', 'w') as f:
            for i in range(len(cost_function_values)):
                f.write(str(cost_function_values[i]))
                f.write('\n')

        fname = f'8qubit/Potential_{n}qubits_{shadow_error}serr_fidelity_{seed}seed_Adam'
        with open(fname+'.txt', 'w') as f:
            for i in range(len(fidelity_values)):
                f.write(str(fidelity_values[i]))
                f.write('\n')

        fname = f'8qubit/Potential_{n}qubits_{shadow_error}serr_params_{seed}seed_Adam'
        with open(fname+'.txt', 'w') as f:
