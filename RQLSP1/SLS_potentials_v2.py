...
while condition==False:
    ...
    if evals%10 == 0:
        fname = f'4qubit/Random1_{n}qubits_{shadow_error}serr_costs_{seed}seed_Adam.txt'
        with open(fname,'w') as f:
            for i in cost_function_values:
                f.write(str(i))
                f.write('\n')

        fname = f'4qubit/Random1_{n}qubits_{shadow_error}serr_fidelity_{seed}seed_Adam.txt'
        with open(fname,'w') as f:
            for i in fidelity_values:
                f.write(str(i))
                f.write('\n')

        fname = f'4qubit/Random1_{n}qubits_{shadow_error}serr_params_{seed}seed_Adam.txt'
        with open(fname,'w') as f:
            for i in params_f:
                f.write(str(i))
                f.write('\n')
