...
while condition==False:
    ...
    if evals%1 == 0:
        ...
        fname = f'4qubit/Random2_{n}qubits_{kappa}kappa_{n_shots}shots_{seed}seed_costs_Adam.txt'
        with open(fname, 'w') as f:
            for i in range(len(cost_function_values)):
                f.write(str(cost_function_values[i]))
                f.write('\n')

        fname = f'4qubit/Random2_{n}qubits_{kappa}kappa_{n_shots}shots_{seed}seed_fidelity_Adam.txt'
        with open(fname, 'w') as f:
            for i in range(len(fidelity_values)):
                f.write(str(fidelity_values[i]))
                f.write('\n')

        fname = f'4qubit/Random2_{n}qubits_{kappa}kappa_{n_shots}shots_{seed}seed_params_Adam.txt'
        with open(fname, 'w') as f:
